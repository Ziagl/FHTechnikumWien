...
void App::init(void)
{
	...
	_gravitation = new Gravity(_gravity);							// initialize gravity

	clan::Vec2f grav;
	grav.x = 0.0f;
	grav.y = -1.0f * START_IMPULSE;
	_startimpuls = new Impulse(grav);								// initialize start impluse for first particle
	_explosion = new Explosion(EXPLOSION_MIN, EXPLOSION_MAX);		// initialize explosion impulse with min and max values
	_friction = new Friction(ICE_KONST);							// initialize friction force with material constant
	//_friction = new Friction(STEEL_KONST);
} // init

void App::render(void)
{
	float y = WINDOW_WIDTH * asin((float)LINE_ANGLE);
	// render floor
	canvas.draw_line(0, FLOOR, WINDOW_WIDTH, FLOOR - y, clan::Colorf::white);
		...
		if ((*it)->_active)
		{
			...
			if ((*it)->_explosionstimer < 0.0f && (*it)->_tiefe < MAX_TIEFE)
			{
				...
				for (int i = 0; i < PARTICLE_NUMBER; ++i)
				{
					...
					f->_speed.x = (*it)->_speed.x;
					f->_speed.y = (*it)->_speed.y;
...
FireworksParticle* App::getFireworksParticle(float x, float y, float lifetime, float size, int tiefe, int timer)
{
	...
	_gravitation->deregisterClass(f);		// deregister, for new gravity force
	_friction->deregisterClass(f);
	_startimpuls->deregisterClass(f);
	_explosion->deregisterClass(f);

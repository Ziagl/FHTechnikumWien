...
void GameObject::render()
{
	...
	{
		for (int i = 1; i < _pathpos; ++i)
		{
			int x = _path[i]._x;
			int y = _path[i]._y;
			float x_koord = static_cast<float>((x*scaleFactor*TILES_SIZE_X) + ((y % 2 == 0) ? TILES_OFFSET_X * scaleFactor : 0));
			float y_koord = static_cast<float>((y*scaleFactor*TILES_OFFSET_Y));
			_pathtile->set_scale(scaleFactor, scaleFactor);
			_pathtile->draw(_canvas, x_koord, y_koord);
		}
	}
